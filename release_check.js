...
['index.html','style.css','app.js','paywall.css','paywall.js','paycode.png'].forEach(need);
const read=f=>fs.readFileSync(path.join(ROOT,f),'utf8');
if(fail.length){console.error('RELEASE_CHECK_FAIL',fail.join(' | '));process.exit(1)}
const html=read('index.html'),app=read('app.js'),pay=read('paywall.js');
const checks=[
  ...
  ['payment-css',html.includes('href="paywall.css"')],
  ['payment-js',html.includes('src="paywall.js"')],
  ['payment-button',html.includes('id="supportBtn"')&&html.indexOf('id="hintBtn"')<html.indexOf('id="supportBtn"')],
  ...
  ['pay-local-state',/localStorage/.test(pay)&&/paycode\.png/.test(pay)]
];
