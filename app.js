...
const PEOPLE={
  gu:{name:'顾曼青',img:'assets/images/interview_gu.jpg',role:'顾文洲之妹。留学归国，与兄长关系长期紧张。',moment:'她补录后没有立刻离开，只把一张旧电影票压在桌上。那是兄妹少年时第一次自己买票看的电影。她说：“我恨过他很久，可我没想让那一下变成他的最后一晚。”'},
  fang:{name:'方正礼',img:'assets/images/interview_fang.jpg',role:'《申江晚报》夜班编辑。负责当夜终版与临时短讯。',moment:'方正礼钱包里一直夹着十年前第一篇署名报道的剪报，纸已经磨得发白。他不是不相信新闻；恰恰因为太相信“版面决定现实”，才一步步把自己放到了事实前面。'},
  su:{name:'苏婉',img:'assets/images/interview_su.jpg',role:'舞厅歌女。与顾文洲相识，近日被卷入许多传闻。',moment:'苏婉要求记者删掉舞厅后台其他姑娘的姓名：“她们今晚还得唱，明天还得活。”她并不怕自己的名字见报，只是不愿别人替她一起付流言的账。'},
  li:{name:'黎月白',img:'assets/images/interview_li.jpg',role:'霞飞路电话总机接线员。当夜负责多路人工转接。',moment:'黎月白随身带一本小册子，记的不是秘密，而是每次断线、误接和被骂的时刻。她说总机员最怕的不是听见不该听的，而是“明明接对了线，最后却没人相信你”。'},
  luo:{name:'罗敬安',img:null,role:'《申江晚报》当夜校对。一直留在编辑室赶终版。',moment:'罗敬安不是核心嫌疑人。他承认自己当时只顾赶版，没有追问消息来源；“方编辑说来源他担，我就把铅字留下了。”这份疏忽被写进内部复盘，但没有被包装成凶案责任。'}
};
...
const HINT_LIBRARY={
  ...
  '2:darkroom':{
    title:'暗房 · 时间线',
    levels:['时间线按钮故意不再显示具体时刻。先翻底片和冲印簿，把每件事的分钟数写在纸上。','时间线需要五个锚点：舞厅、医生、顾宅门厅、外线、巡捕。它们的具体时刻分别散在不同原件里。','顺序是：舞厅后台 → 医生离宅 → 顾宅门厅 → 公用电话外线 → 巡捕抵达。']
  },
  '3:newsroom':{
    title:'报馆 · 离岗记录',
  ...
  '4:finale':{
    title:'终稿 · 责任链',
    levels:['四个下拉框问的是四段不同的行为，不要求所有责任都落在同一个人身上。','先确定最初伤害，再单独判断后来“没有求救”“整理现场”“送出短讯”的行为主体。','最初伤害对应顾曼青的补录；其余三段在方正礼第二份口供中均有直接承认，并由法医复核解释“不求救”的独立意义。']
  },
...
function freshState(expert=false){
  ...
  return {stage:1,scene:'newsroom',expert,sound:true,evidence:[],facts:[],interviews:{gu:false,li:false,su:false,fang:false},solved:{anomaly:false,route:false,timeline:false,final:false,review:false},hintUse:{1:0,2:0,3:0,4:0},hintHistory:{},timelinePick:[],routePick:[],ending:null,completedOnce:false,shuffleSeed:Math.floor(Math.random()*999999),reviewVariant:expert?variants[Math.floor(Math.random()*variants.length)]:'phone'};
}
...
function migrateState(v){
  ...
  n.solved=Object.fromEntries(Object.entries(Object.assign({anomaly:false,route:false,timeline:false,final:false,review:false},v?.solved||{})).filter(([k])=>['anomaly','route','timeline','final','review'].includes(k)).map(([k,val])=>[k,!!val]));
...
function objective(){
  if(state.expert){const r=REVIEW_VARIANTS[state.reviewVariant];return state.stage<4?`独立复核：不显示程序提示。终局追加“${r.title.replace('复核任务 · ','')}”。`:'独立复核：完成终稿判断与随机复核。'}
  if(state.stage===1)return '从目前开放的场景翻阅原始材料，写下一处能够被纸面记录直接支持的异常。';
  if(state.stage===2)return state.solved.route?'电话线路已归档。继续整理当夜事件顺序。':'调查范围扩展到电话局与暗房；先完成当前能出现的操作题。';
...
function renderFinale(){
  if(!state.solved.final)return `<section class="puzzle"><h4>责任链 · 四段行为</h4><p>根据已经归档的原件与补录，分别选择四段行为的主要责任人。不要默认四项必须是同一个人。</p><div class="final-grid">${finalItem('fin1','最初伤害')}${finalItem('fin2','延误救助')}${finalItem('fin3','伪造“自尽”现场')}${finalItem('fin4','00:27提前送讯')}</div><button class="action-btn" data-action="check-final">提交责任链</button></section>`;
...
function renderExpertReview(){
  const r=REVIEW_VARIANTS[state.reviewVariant];return `<section class="puzzle"><h4>${r.title}</h4><p>${r.question}</p><div class="question-list">${r.options.map(([label,,flag])=>`<button class="question-btn" data-review-answer="${flag==='correct'?'1':'0'}"><strong>${label}</strong></button>`).join('')}</div></section>`;
}
...
async function runQA(){
  ...
  try{
    ...
    if(!HINT_LIBRARY['1:newsroom']||!HINT_LIBRARY['2:switchboard']||!HINT_LIBRARY['3:interviews']||!HINT_LIBRARY['4:finale'])errors.push('hint-contexts');
